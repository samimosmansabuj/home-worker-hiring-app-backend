...
class CustomerOrderViewSet(UpdateModelViewSet):
    ...
    def perform_create(self, serializer):
        order = serializer.save(customer=self.request.user.hasCustomerProfile)
        return order
    ...
    def assign_provider(self, request, pk=None):
        ...
        if order.status != OrderStatus.PENDING:
            raise ValidationError("Only pending order can assign provider")
    ...
    def make_payment(self, request, pk=None):
        ...
        if order.status != OrderStatus.ACCEPT:
            raise ValidationError("Order must be accepted first")
    ...
    def refund_request(self, request, pk=None):
        ...
        if order.status not in [OrderStatus.CONFIRM, OrderStatus.IN_PROGRESS]:
            raise ValidationError("Refund not allowed now")

        serializer = OrderRefundRequestSerializer(
            data=request.data,
